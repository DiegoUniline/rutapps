...
import TeamAccessDialog from '@/components/admin/TeamAccessDialog';
import {
...
const EMPTY_MEMBER: MemberDraft = {
  id: null, userId: null, name: '', email: '', emailIsFictitious: false, phone: '', jobTitle: 'Ejecutivo comercial', managerId: '',
...
export default function AdminTeamTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const workspace = result as TeamWorkspace;
      const { data: emailModes } = await (supabase as any)
        .from('platform_team_members')
        .select('person_id,email_is_fictitious');
      const modeByPerson = new Map<string, boolean>((emailModes ?? []).map((row: { person_id: string; email_is_fictitious: boolean }) => [row.person_id, Boolean(row.email_is_fictitious)]));
      workspace.members = (workspace.members ?? []).map(member => ({
        ...member,
        email_is_fictitious: modeByPerson.get(member.id) ?? false,
      }));
      setData(workspace);
  ...
  const openEdit = (member: TeamMember) => {
    setDraft({
      id: member.id, userId: member.user_id, name: member.name, email: member.email,
      emailIsFictitious: Boolean(member.email_is_fictitious), phone: member.phone ?? '',
  ...
  const saveMember = async () => {
    ...
    setSaving(true);
    try {
      const { data: saved, error } = await rpcClient.rpc('admin_save_team_member', {
      ...
      if (error) throw error;
      const personId = draft.id || (saved as { person_id?: string } | null)?.person_id;
      if (!personId) throw new Error('No se pudo identificar al integrante guardado');
      const { error: modeError } = await (supabase as any)
        .from('platform_team_members')
        .update({ email_is_fictitious: draft.emailIsFictitious })
  ...
  const grantAccess = (member: TeamMember) => setAccessMember(member);
  ...
  if (personId) {
    ...
    return <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/[0.08] via-card to-card p-5 sm:p-6"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-black">{selected.name}</h1><MemberStatus member={selected} />{selected.email_is_fictitious && <Badge variant="outline">Correo ficticio</Badge>}<Badge variant="outline">{TEAM_LEVEL_LABELS[selected.access_level]}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{selected.job_title || 'Sin puesto'} · {selected.email} · {TEAM_SCOPE_LABELS[selected.access_scope]}</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => openEdit(selected)}><Pencil className="mr-2 h-4 w-4" />Editar</Button><Button onClick={() => grantAccess(selected)}>{selected.user_id ? <ShieldCheck className="mr-2 h-4 w-4" /> : <Send className="mr-2 h-4 w-4" />}{selected.user_id ? 'Cambiar contraseña' : 'Dar acceso'}</Button></div></div></section>
  ...
  function renderDialogs() {
    return <>
      ...
      <TeamAccessDialog member={accessMember} onClose={() => setAccessMember(null)} onCompleted={load} />
