...
function LeadDetailDialog({
  lead, assignees, open, onOpenChange, onUpdated, onOffer, onDelete,
}: {
  lead: TrialCrmLead | null;
  ...
  useEffect(() => {
    ...
    rpcClient.rpc('fn_admin_trial_crm_history', { p_empresa_id: lead.empresa_id })
      .then(({ data, error }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Target className="h-5 w-5 text-primary" /> {lead.nombre}</DialogTitle>
          <DialogDescription>Licencia {lead.licencia || 'sin asignar'} · Registrada hace {lead.age_days} días · Cero ventas operativas</DialogDescription>
        </DialogHeader>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-xl border border-border p-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div><p className="font-bold">Contacto directo</p><p className="text-xs text-muted-foreground">{lead.telefono || 'Sin teléfono'} · {lead.email || 'Sin correo'}</p></div>
                <ContactButtons lead={lead} />
              </div>
            </div>

            <div className="rounded-xl border border-border p-4 space-y-4">
              <div><h4 className="font-bold text-sm">Registrar seguimiento</h4><p className="text-[11px] text-muted-foreground">Cada llamada, mensaje o nota queda en la bitácora.</p></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><Label>Etapa</Label><select value={stage} onChange={e => setStage(e.target.value as TrialCrmStage)} className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm">{TRIAL_CRM_STAGES.map(value => <option key={value} value={value}>{TRIAL_CRM_STAGE_INFO[value].label}</option>)}</select></div>
                <div><Label>Responsable</Label><select value={assignedTo} onChange={e => setAssignedTo(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">Sin asignar</option>{assignees.map(person => <option key={person.id} value={person.id}>{person.name}</option>)}</select></div>
                <div><Label>Actividad</Label><select value={activityType} onChange={e => setActivityType(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="call">Llamada</option><option value="whatsapp">WhatsApp</option><option value="email">Correo</option><option value="note">Nota interna</option><option value="stage">Cambio de etapa</option><option value="assignment">Asignación</option></select></div>
                <div><Label>Resultado</Label><Input value={outcome} onChange={e => setOutcome(e.target.value)} className="mt-1" placeholder="Ej. Contestó, llamar el viernes" /></div>
                <div><Label>Próximo seguimiento</Label><Input type="datetime-local" value={nextFollowUp} onChange={e => setNextFollowUp(e.target.value)} className="mt-1" /></div>
                <div className="sm:col-span-2"><Label>Nota</Label><Textarea value={note} onChange={e => setNote(e.target.value)} className="mt-1" rows={3} placeholder="Qué necesita, por qué no probó, objeción y siguiente paso…" /></div>
              </div>
              <div className="flex justify-end"><Button onClick={save} disabled={saving}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />} Guardar seguimiento</Button></div>
            </div>

            <div className="rounded-xl border border-border overflow-hidden">
              <div className="flex items-center gap-2 border-b border-border p-4"><History className="h-4 w-4 text-primary" /><h4 className="font-bold text-sm">Historial comercial</h4></div>
              {historyLoading ? <div className="flex justify-center py-10"><Loader2 className="h-5 w-5 animate-spin" /></div> : history.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">Todavía no hay seguimientos.</p> : <div className="divide-y divide-border">{history.map(item => <div key={item.id} className="p-3 text-xs"><div className="flex items-center justify-between gap-2"><span className="font-bold capitalize">{item.activity_type.replace('_', ' ')}</span><span className="text-muted-foreground">{dateLabel(item.created_at, 'dd MMM yyyy HH:mm')}</span></div>{item.outcome && <p className="mt-1 font-medium">{item.outcome}</p>}{item.note && <p className="mt-1 whitespace-pre-wrap text-muted-foreground">{item.note}</p>}{item.new_stage && <p className="mt-1 text-[10px] text-muted-foreground">Etapa: {TRIAL_CRM_STAGE_INFO[item.new_stage as TrialCrmStage]?.label ?? item.new_stage}</p>}</div>)}</div>}
            </div>
          </div>

          <div className="space-y-4">
            <div className="rounded-xl border border-border p-4">
              <h4 className="font-bold text-sm">Huella de activación</h4>
              <div className="mt-3 h-2 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-primary" style={{ width: `${lead.activation_score}%` }} /></div>
              <p className="mt-1 text-right text-xs font-bold">{lead.activation_score}/100</p>
              <div className="mt-3 space-y-2 text-xs">
                <p className="flex justify-between"><span>Productos</span><b>{fmtNum(lead.products)}</b></p><p className="flex justify-between"><span>Clientes capturados</span><b>{fmtNum(lead.clients)}</b></p><p className="flex justify-between"><span>Ventas borrador</span><b>{fmtNum(lead.draft_sales)}</b></p><p className="flex justify-between"><span>Usuarios</span><b>{fmtNum(lead.users)}</b></p><p className="flex justify-between"><span>Último acceso</span><b>{dateLabel(lead.last_sign_in_at)}</b></p>
              </div>
            </div>
            <div className="rounded-xl border border-violet-500/25 bg-violet-500/[0.04] p-4">
              <div className="flex items-center gap-2 text-violet-700"><Gift className="h-4 w-4" /><h4 className="font-bold text-sm">Oferta de recuperación</h4></div>
              {lead.coupon_active ? <div className="mt-3"><p className="font-mono text-lg font-black">{lead.coupon_code}</p><p className="text-xs text-muted-foreground">{lead.coupon_discount_pct}% por {lead.coupon_months} mes(es) · vence {dateLabel(lead.coupon_expires_at)}</p></div> : <p className="mt-2 text-xs text-muted-foreground">{canOffer ? 'Crea un cupón individual, limitado a esta empresa y aplicado en su siguiente checkout.' : 'Cuenta protegida: ya tiene pago o una suscripción activa/vencida en Stripe.'}</p>}
              <Button className="mt-3 w-full" size="sm" variant="outline" disabled={!canOffer} onClick={() => onOffer(lead)}><Sparkles className="mr-1.5 h-3.5 w-3.5" /> {lead.coupon_active ? 'Reemplazar oferta' : 'Crear oferta'}</Button>
            </div>
            <div className="rounded-xl border border-border p-4">
              <p className="text-[10px] font-bold uppercase text-muted-foreground">Suscripción</p><p className="mt-1 font-bold">{SUBSCRIPTION_LABELS[lead.subscription_status] || lead.subscription_status}</p>
              <p className="mt-2 text-xs text-muted-foreground">Prueba: {dateLabel(lead.trial_ends_at)}<br />Facturas pagadas: {lead.paid_invoices}<br />Stripe: {lead.stripe_subscription_id ? 'Vinculada' : 'Sin suscripción'}</p>
            </div>
            <div className={cn('rounded-xl border p-4', canDelete ? 'border-red-500/30 bg-red-500/[0.035]' : 'border-border bg-muted/20')}>
              <div className="flex items-center gap-2"><ShieldCheck className={cn('h-4 w-4', canDelete ? 'text-red-600' : 'text-muted-foreground')} /><p className="font-bold text-sm">Depuración protegida</p></div>
              <p className="mt-2 text-[11px] text-muted-foreground">{canDelete ? 'Ya cumple los requisitos para solicitar eliminación total.' : 'Sólo se habilita sin ventas, pagos ni Stripe y después de marcarlo No interesado o Descartado.'}</p>
              <Button className="mt-3 w-full" size="sm" variant="destructive" disabled={!canDelete} onClick={() => onDelete(lead)}><Trash2 className="mr-1.5 h-3.5 w-3.5" /> Eliminar empresa y datos</Button>
            </div>
          </div>
        </div>
      </DialogContent>
...
export default function AdminTrialCrmTab() {
  ...
  const [selectedLead, setSelectedLead] = useState<TrialCrmLead | null>(null);
  ...
  const openLead = (lead: TrialCrmLead) => { setSelectedLead(lead); setDetailOpen(true); };
  ...
  const deleteCandidate = async () => {
    ...
    try {
      ...
      setDeleteLead(null); setDetailOpen(false); setSelectedLead(null);
      await load();
    } catch (error) { toast.error(errorText(error)); }
  ...
  return (
    <div className="space-y-4">
      ...
      <LeadDetailDialog lead={selectedLead} assignees={snapshot?.assignees ?? []} open={detailOpen} onOpenChange={setDetailOpen} onUpdated={load} onOffer={openOffer} onDelete={openDelete} />
