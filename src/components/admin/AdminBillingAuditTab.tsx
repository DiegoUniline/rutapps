            ...
            <div className="overflow-x-auto max-h-[calc(100dvh-315px)]">
              <Table className="min-w-[1500px]">
                <TableHeader className="sticky top-0 z-10 bg-card">
                  <TableRow>
                    <TableHead className="w-[230px]">Empresa</TableHead>
                    <TableHead>Alta y prueba</TableHead>
                    <TableHead>RutApp</TableHead>
                    <TableHead>Stripe</TableHead>
                    <TableHead>Periodo esperado</TableHead>
                    <TableHead>Último cobro</TableHead>
                    <TableHead>Tarjeta asociada</TableHead>
                    <TableHead className="w-[245px]">ID suscripción Stripe</TableHead>
                  ...
                  {filtered.map(row => (
                    ...
                    >
                      ...
                      <TableCell className="text-xs">
                        <DateLine label="Alta" value={row.expected.signup_date} />
                        <DateLine label="Fin prueba" value={row.expected.trial_end} strong />
                        {row.db_subscription?.trial_ends_at && (
                          <DateLine label="RutApp" value={row.db_subscription.trial_ends_at} muted />
                        )}
                      </TableCell>
                      <TableCell>
...
function AuditDateBox({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={cn('rounded-lg border p-2.5', accent && 'border-primary/40 bg-primary/5')}>
      <div className="text-[10px] text-muted-foreground">{label}</div>
      <div className="text-sm font-semibold mt-0.5">{fmtDate(value)}</div>
    </div>
  );
}
