      ...
      <table className="w-full text-[13px]">
        ...
        <tbody>
          {cfdis.map((cfdi: any) => {
            ...
            return (
              <tr key={cfdi.id} className="border-b border-table-border hover:bg-table-hover cursor-pointer" onClick={() => navigate(`/facturacion-cfdi/${cfdi.id}`, { state: { returnTo: `/ventas/${ventaId}` } })}>
