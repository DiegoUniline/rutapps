  ...
  return useQuery({
    ...
    queryFn: async () => {
      return fetchAllPages((from, to) => {
        let q = supabase
          .from('entregas')
          .select('id, folio, fecha, fecha_entrega, status, notas, pedido_id, vendedor_id, cliente_id, almacen_id, vendedor_ruta_id, fecha_asignacion, fecha_carga, validado_at, clientes(nombre), vendedores:profiles!entregas_vendedor_id_profiles_fkey(nombre, telefono, almacen_destino:almacenes!profiles_almacen_id_fkey(id, nombre)), ventas!entregas_pedido_id_fkey(folio, fecha), almacenes(nombre), vendedor_ruta:profiles!entregas_vendedor_ruta_id_profiles_fkey(nombre, telefono, almacen_destino:almacenes!profiles_almacen_id_fkey(id, nombre)), entrega_lineas(id, producto_id, cantidad_pedida, cantidad_entregada, hecho, almacen_origen_id, productos(codigo, nombre), almacenes:almacen_origen_id(id, nombre))')
