...
export function useVendedores() {
  return useQuery({ queryKey: ['vendedores'], staleTime: CATALOG_STALE, queryFn: async () => { const { data } = await supabase.from('vendedores').select('id, nombre').eq('activo', true as any).order('nombre'); return data as Vendedor[]; }});
}
