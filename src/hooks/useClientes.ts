    ...
    queryFn: async () => {
      ...
      const rows = await fetchAllPages<any>((from, to) => {
        let q = supabase.from('clientes')
          .select('id, codigo, nombre, telefono, contacto, email, rfc, direccion, colonia, vendedor_id, cobrador_id, zona_id, tarifa_id, lista_id, lista_precio_id, status, orden, credito, limite_credito, dias_credito, dia_visita, gps_lat, gps_lng, frecuencia, foto_url, foto_fachada_url')
