  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('productos')
        .select('id, codigo, nombre, precio_principal, costo, cantidad, status, imagen_url, tiene_iva, iva_pct, tiene_ieps, ieps_pct, min, marca_id, marcas(nombre), clasificacion_id, clasificaciones(nombre), proveedor_id, proveedores(nombre), unidad_venta_id, lista_id, listas(nombre)')
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('productos')
        .select('id, codigo, nombre, precio_principal, costo, unidad_venta_id, unidad_compra_id, tiene_iva, tiene_ieps, tasa_iva_id, tasa_ieps_id, iva_pct, ieps_pct, ieps_tipo, costo_incluye_impuestos, unidades_venta:unidades!productos_unidad_venta_id_fkey(nombre, abreviatura)')
