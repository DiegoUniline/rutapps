...
export function useAlmacenes(opts?: { includeMermas?: boolean }) {
  ...
  return useQuery({
    queryKey: ['almacenes', empresa?.id, includeMermas],
    staleTime: CATALOG_STALE,
    enabled: !!empresa?.id,
    networkMode: 'always',
    queryFn: async () => {
      const readCache = async () => {
        try {
          ...
          const cached = await offlineDb.almacenes.where('empresa_id').equals(empresa!.id).toArray();
          return (cached as any[])
            .filter(a => a.activo !== false && (includeMermas || !a.es_merma))
            .map(a => ({ id: a.id, nombre: a.nombre, es_merma: !!a.es_merma }))
            .sort((a, b) => String(a.nombre || '').localeCompare(String(b.nombre || '')));
        } catch { return []; }
      };
      if (typeof navigator !== 'undefined' && navigator.onLine === false) {
        const c = await readCache(); if (c.length) return c as unknown as Almacen[];
      }
      try {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const rows = await fetchAllPages<any>((from, to) => supabase.from('productos')
        .select('id, codigo, nombre, nombre_compra, nombre_venta, nombre_ticket, formula, precio_principal, costo, cantidad, clasificacion_id, unidad_venta_id, unidad_compra_id, factor_conversion, tiene_iva, tiene_ieps, iva_pct, ieps_pct, ieps_tipo, costo_incluye_impuestos, es_granel, unidad_granel, vender_sin_stock, usa_listas_precio, maneja_lote')
        .eq('empresa_id', empresa!.id)
        .eq('status', 'activo')
        .order('nombre')
        .range(from, to));
