...
export function useVentasPaginated(search?: string, statusFilter?: string, tipoFilter?: string, page = 1, pageSize = 80, condicionFilter?: string, vendedorFilter?: string, dateFrom?: string, dateTo?: string, fetchAll = false) {
  ...
  return useQuery({
    ...
    enabled: !!empresa?.id,
    queryFn: async () => {
      const SELECT = 'id, folio, fecha, created_at, total, subtotal, iva_total, descuento_total, descuento_extra, descuento_extra_tipo, saldo_pendiente, status, tipo, condicion_pago, vendedor_id, cliente_id, almacen_id, es_saldo_inicial, origen, clientes(nombre, rfc, telefono, direccion, colonia), vendedores:profiles!vendedor_id(nombre, telefono), almacenes(nombre)';
