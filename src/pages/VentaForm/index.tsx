...
export default function VentaFormPage() {
  ...
  const handlePrintTicket = () => {
    ...
    const td = buildTicketDataFromVenta({
      ...
      pagos: (pagosData ?? []).map((p: any) => ({ metodo: (p.cobros as any)?.metodo_pago ?? 'efectivo', monto: Number(p.monto_aplicado ?? 0), fecha: (p.cobros as any)?.fecha ?? '', referencia: (p.cobros as any)?.referencia })),
    });
