...
import { VentaGastosTab } from '@/components/venta/VentaGastosTab';
  ...
  const handlePrintTicket = () => {
    ...
    const td = buildTicketDataFromVenta({
      ...
      lineas: lineas.filter(l => l.producto_id).map(l => ({
        ...
        precio_lista_unitario: (l as any).precio_lista_unitario,
        descuento_promocion_monto: Number((l as any).descuento_promocion_monto ?? 0),
        descuento_manual_monto: Number((l as any).descuento_manual_monto ?? 0),
        iva_pct: Number((l as any).iva_pct ?? 0),
        ieps_pct: Number((l as any).ieps_pct ?? 0),
      })),
    ...
    <div className="min-h-full">
      ...
      <div className="w-full max-w-none p-2.5 sm:p-3 space-y-2.5">
        <div className="bg-card border border-border rounded-md p-3">
          ...
          {!isNew && form.tipo !== 'saldo_inicial' && Number(form.total) > 0 && lineas.filter(l => l.producto_id).length === 0 && (
        ...
        <div className="bg-card border border-border rounded-md">
          <OdooTabs tabs={[
            ...
            ...(!isNew ? [{ key: 'gastos', label: `Gastos (${gastosCount ?? 0})`, content: <VentaGastosTab ventaId={form.id!} /> }] : []),
