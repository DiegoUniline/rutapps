...
export function useDashboardAging() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (vIds.length > 0) {
        const { data: profs } = await supabase.from('profiles').select('id, nombre, full_name').in('id', vIds);
        vendMap = new Map((profs ?? []).map((p: any) => [p.id, p.nombre || p.full_name || 'Sin nombre']));
      }
