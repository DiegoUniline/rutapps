...
export default function RutaVentaDetalle() {
  ...
  if (h.view === 'editar') {
    return <EditarView venta={h.venta} editLineas={h.editLineas} editCondicion={h.editCondicion} setEditCondicion={h.setEditCondicion} editNotas={h.editNotas} setEditNotas={h.setEditNotas} editTotals={h.editTotals} clienteData={h.clienteData} saldoPendienteOtras={h.saldoPendienteOtras} creditoDisponible={h.creditoDisponible} excedeCredito={h.excedeCredito} saving={h.saving} showProductSearch={h.showProductSearch} setShowProductSearch={h.setShowProductSearch} searchProducto={h.searchProducto} setSearchProducto={h.setSearchProducto} filteredProductos={h.filteredProductos} addProductToEdit={h.addProductToEdit} updateEditQty={h.updateEditQty} removeEditLine={h.removeEditLine} handleSaveEdits={h.handleSaveEdits} onBack={() => h.setView('detalle')} fmt={h.fmt} empresaId={h.empresa?.id} manejaLotesEmpresa={h.manejaLotesEmpresa} productoManejaLote={h.productoManejaLote} lotePendienteEdit={h.lotePendienteEdit} setEditLineaLotes={h.setEditLineaLotes} almacenLotesBase={h.almacenLotesBase} />;
  }

  if (h.view === 'cobrar') {
    return <CobrarView venta={h.venta} clienteNombre={clienteNombre} saldoActual={h.saldoActual} cuentasPendientes={h.cuentasPendientes} totalAplicarOtras={h.totalAplicarOtras} montoAplicarActual={h.montoAplicarActual} totalACobrar={h.totalACobrar} metodoPago={h.metodoPago} setMetodoPago={h.setMetodoPago} saldoFavorDisp={h.saldoFavorDisp} montoRecibido={h.montoRecibido} setMontoRecibido={h.setMontoRecibido} referenciaPago={h.referenciaPago} setReferenciaPago={h.setReferenciaPago} cambio={h.cambio} saving={h.saving} handleCobrar={h.handleCobrar} updateCuentaMonto={h.updateCuentaMonto} updateMontoAplicarActual={h.updateMontoAplicarActual} liquidarTodas={h.liquidarTodas} onBack={() => h.setView('detalle')} fmt={h.fmt} empresaId={h.empresa?.id} manejaLotesEmpresa={h.manejaLotesEmpresa} productoManejaLote={h.productoManejaLote} lotePendienteEdit={h.lotePendienteEdit} setEditLineaLotes={h.setEditLineaLotes} almacenLotesBase={h.almacenLotesBase} />;
  }
