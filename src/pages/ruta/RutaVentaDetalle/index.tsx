...
export default function RutaVentaDetalle() {
  ...
  return <DetalleView venta={h.venta} clienteNombre={clienteNombre} vendedorNombre={vendedorNombre} clienteData={h.clienteData} clienteId={h.clienteId} lineas={lineas} empresa={h.empresa} ecPdfBlob={h.ecPdfBlob} showEcPreview={h.showEcPreview} setShowEcPreview={h.setShowEcPreview} showWADialog={h.showWADialog} setShowWADialog={h.setShowWADialog} waPhone={h.waPhone} setWaPhone={h.setWaPhone} sendingWA={h.sendingWA} saving={h.saving} handleWhatsAppSend={h.handleWhatsAppSend} handleDownloadPDF={h.handleDownloadPDF} handleEstadoCuenta={h.handleEstadoCuenta} initEditar={h.initEditar} initCobrar={h.initCobrar} handleCancelar={h.handleCancelar} onBack={() => h.navigate(-1)} fmt={h.fmt} />;
}
