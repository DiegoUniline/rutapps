    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="flex-1 overflow-auto px-3 py-3 space-y-3 pb-24">
        <section className="bg-card rounded-xl border border-border p-3.5">
          ...
          <div className="flex justify-between items-baseline"><span className="text-[13px] text-foreground">{p.venta.folio ?? 'Sin folio'}</span><span className="text-[18px] font-bold text-foreground">${p.fmt(p.saldoActual)}</span></div>
        </section>
        {p.cuentasPendientes.length > 0 && <CuentasPendientesSection {...p} />}
        <MetodoPagoSection {...p} />
        <ResumenSection {...p} />
      </div>
...
function CuentasPendientesSection({ cuentasPendientes, updateCuentaMonto, liquidarTodas, totalAplicarOtras, fmt }: Props) {
  return (
    <section className="bg-card rounded-xl border border-border p-3.5">
      ...
      {totalAplicarOtras > 0 && <div className="mt-2 pt-2 border-t border-border/60 flex justify-between"><span className="text-[11px] text-muted-foreground">Total a cuentas anteriores</span><span className="text-[12px] font-bold text-foreground">${fmt(totalAplicarOtras)}</span></div>}
    </section>
  );
}

function MetodoPagoSection({ metodoPago, setMetodoPago, montoRecibido, setMontoRecibido, referenciaPago, setReferenciaPago, totalACobrar, cambio, fmt }: Props) {
  return (
    <section className="bg-card rounded-xl border border-border p-3.5">
...
function ResumenSection({ saldoActual, totalAplicarOtras, totalACobrar, fmt }: Props) {
  return (
    <section className="bg-card rounded-xl border border-border p-3.5">
      <div className="space-y-1">
        <div className="flex justify-between text-[12px]"><span className="text-muted-foreground">Saldo de esta venta</span><span className="font-medium text-foreground tabular-nums">${fmt(saldoActual)}</span></div>
        {totalAplicarOtras > 0 && <div className="flex justify-between text-[12px]"><span className="text-muted-foreground">Cuentas anteriores</span><span className="font-medium text-foreground tabular-nums">${fmt(totalAplicarOtras)}</span></div>}
      </div>
      <div className="flex justify-between items-baseline mt-2 pt-2 border-t border-border/60"><span className="text-[13px] font-semibold text-foreground">Total a cobrar</span><span className="text-[20px] font-bold text-primary tabular-nums">${fmt(totalACobrar)}</span></div>
    </section>
  );
}
