...
import type { DevolucionItem, AccionDevolucion } from './types';
        ...
        {filteredDevProductos?.map(p => {
          ...
          return (
            <div key={p.id} className={`rounded-lg px-3 py-2 transition-all ${qty > 0 ? 'bg-destructive/[0.04] ring-1 ring-destructive/20' : 'bg-card'}`}>
              ...
              {/* Motivo + Acción selectors */}
              {qty > 0 && dev && (
                <div className="mt-2 space-y-1.5">
                  {/* Motivo */}
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-muted-foreground w-12 shrink-0">Motivo</span>
                    <select
                      value={dev.motivo}
                  ...
                  {/* Acción */}
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-muted-foreground w-12 shrink-0">Acción</span>
                    <select
                      value={dev.accion}
                      onChange={e => updateDevAccion(p.id, e.target.value as AccionDevolucion)}
                      className="flex-1 bg-accent/40 rounded-lg px-2 py-1.5 text-[11px] text-foreground focus:outline-none focus:ring-1.5 focus:ring-primary/40"
                    >
                      {ACCIONES.map(a => <option key={a.value} value={a.value}>{a.icon} {a.label}</option>)}
                    </select>
                  </div>

                  {/* Reemplazo (solo reposición) */}
                  {dev.accion === 'reposicion' && (
                    <button
                      onClick={() => setShowReemplazoFor(p.id)}
                      className="w-full mt-0.5 bg-primary/5 border border-primary/20 rounded-lg px-3 py-2 text-left active:scale-[0.98] transition-all"
                    >
                      <div className="flex items-center gap-1.5">
                        <ArrowRightLeft className="h-3.5 w-3.5 text-primary shrink-0" />
                        {dev.reemplazo_nombre ? (
                          <span className="text-[11px] font-medium text-primary truncate">{dev.reemplazo_nombre}</span>
                        ) : (
                          <span className="text-[11px] text-muted-foreground">Seleccionar producto de reemplazo...</span>
                        )}
                      </div>
                    </button>
                  )}
                  ...
                  {/* Info de acción */}
                  <p className="text-[9px] text-muted-foreground/70 italic pl-0.5">
                    {ACCIONES.find(a => a.value === dev.accion)?.desc}
                    {(dev.accion === 'nota_credito' || dev.accion === 'descuento_venta' || dev.accion === 'devolucion_dinero') && (
                      <> · <span className="font-semibold text-foreground/60">${fmt(dev.precio_unitario * dev.cantidad)}</span></>
                    )}
                  </p>
                </div>
              )}
