...
import { ArrowLeft, Truck, Camera, Loader2, MapPin, Play, CheckCircle2 } from 'lucide-react';
...
export default function RutaIniciarPage() {
  ...
  const submit = async () => {
    if (!vehiculoId) return toast.error('Selecciona un vehículo');
    const km = parseFloat(kmInicio);
    if (!Number.isFinite(km) || km < 0) return toast.error('Captura un KM válido');
    if (!foto) return toast.error('Toma la foto del odómetro');

    try {
      setUploading(true);
      const fotoUrl = await uploadOdometroFoto(foto, empresa!.id, 'inicio');
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-3 pb-3 pt-1 bg-gradient-to-t from-background via-background to-transparent">
        <button
          onClick={submit}
          disabled={uploading || abrir.isPending || !vehiculoId || !foto || !kmInicio}
