...
import { useProducto, useSaveProducto, useDeleteProducto, useMarcas, useProveedores, useClasificaciones, useListas, useUnidades, useTasasIva, useTasasIeps, useAlmacenes, useUnidadesSat, useTarifasForSelect, useTarifaLineasForProducto, useSaveProductoProveedor, useDeleteProductoProveedor } from '@/hooks/useData';
                      ...
                      <td className="py-1.5 px-3 text-right" onClick={cellClick('comision')}>
                        {isEditing && editingCol === 'comision' ? (
                          <input
                            ...
                            value={currentVals.comision_pct || ''}
                            onChange={e => setEditVal((p: any) => ({ ...p, comision_pct: +e.target.value }))}
                            onBlur={handleBlur}
                            onKeyDown={e => { if (e.key === 'Enter') handleBlur(); if (e.key === 'Escape') { setEditingId(null); setEditingCol(null); } }}
                          />
                        ) : (
