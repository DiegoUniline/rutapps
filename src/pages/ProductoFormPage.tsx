...
function ListasPrecioProductoTab({ productoId, isNew, tarifasDisp }: {
  ...
  (precioLineas ?? []).forEach((pl: any) => {
    ...
    byTarifa.get(tarifaId)!.listas.push({
      listaId: lp.id,
      listaNombre: lp.nombre,
      es_principal: lp.es_principal,
      lineaId: pl.id,
      precio: pl.precio,
    });
  });

  // Also fetch all tarifas with their listas for adding prices
  ...
  const handleSavePrices = async () => {
    ...
    try {
      for (const [listaId, precio] of Object.entries(addPrices)) {
        if (precio > 0) {
          await savePrecio.mutateAsync({ lista_precio_id: listaId, producto_id: productoId, precio });
        }
      }
      setAddingTarifaId(null);
      setAddPrices({});
      refetch();
  ...
  const handleUpdatePrecio = async (lineaId: string, listaId: string, precio: number) => {
    ...
    try {
      await savePrecio.mutateAsync({ id: lineaId, lista_precio_id: listaId, producto_id: productoId, precio });
      refetch();
    } catch (err: any) { toast.error(err.message); }
  };
  ...
  return (
    <div className="space-y-3">
      {/* Existing prices grouped by tarifa */}
      {Array.from(byTarifa.entries()).map(([tarifaId, { tarifaNombre, listas: ls }]) => (
        <div key={tarifaId} className="border border-border rounded">
          <div className="px-3 py-1.5 bg-muted/30 border-b border-border text-[12px] font-semibold text-foreground">
            {tarifaNombre}
          </div>
          <table className="w-full text-sm">
            <tbody>
              {ls.map(l => (
                <tr key={l.listaId} className="border-b border-table-border last:border-0 hover:bg-table-hover">
                  ...
                  <td className="py-1.5 px-3 text-right w-40">
                    <InlinePriceEdit
                      value={l.precio}
                      onSave={(val) => l.lineaId && handleUpdatePrecio(l.lineaId, l.listaId, val)}
                    />
                  </td>
      ...
      {byTarifa.size === 0 && !addingTarifaId && (
        <div className="py-4 text-center text-[12px] text-muted-foreground border border-border rounded">
          Sin precios por lista configurados. Agrega precios desde una tarifa.
        </div>
      )}

      {/* Add prices from a tarifa */}
        ...
        <div className="border border-primary/30 rounded bg-primary/5 p-3 space-y-2">
          ...
          {addingListas.map(l => (
            <div key={l.id} className="flex items-center gap-3">
              ...
              <input
                type="number"
                className="inline-edit-input w-28 text-right"
                placeholder="$ 0.00"
                value={addPrices[l.id] || ''}
                onChange={e => setAddPrices(p => ({ ...p, [l.id]: +e.target.value }))}
                step="0.01"
              />
            </div>
          ))}
          {addingListas.length === 0 && (
            <div className="text-[12px] text-muted-foreground">Esta tarifa no tiene listas de precios. Créalas primero en la tarifa.</div>
          )}
...
function InlinePriceEdit({ value, onSave }: { value: number; onSave: (v: number) => void }) {
  ...
  if (editing) {
    return (
      <input
        autoFocus
        type="number"
        className="inline-edit-input text-right w-full"
        value={draft}
  ...
  return (
    <span className="inline-edit-cell inline-edit-idle font-mono text-odoo-teal font-semibold cursor-pointer" onClick={() => { setDraft(value.toString()); setEditing(true); }}>
      $ {value.toFixed(2)}
    </span>
  );
}
