...
/* ── Listas de Precios Tab (product form) ── */
function ListasPrecioProductoTab({ productoId, isNew, tarifasDisp }: { productoId?: string; isNew: boolean; tarifasDisp: any }) {
  const { data: precioLineas, refetch } = useListaPrecioLineasForProducto(isNew ? undefined : productoId);
  const savePrecio = useSaveListaPrecioLinea();
  const deletePrecio = useDeleteListaPrecioLinea();
  const byTarifa = new Map<string, { tarifaNombre: string; listas: { listaId: string; listaNombre: string; es_principal: boolean; lineaId?: string; precio: number }[] }>();
  (precioLineas ?? []).forEach((pl: any) => {
    const lp = pl.lista_precios; if (!lp?.tarifas) return;
    const tid = lp.tarifas.id;
    if (!byTarifa.has(tid)) byTarifa.set(tid, { tarifaNombre: lp.tarifas.nombre, listas: [] });
    byTarifa.get(tid)!.listas.push({ listaId: lp.id, listaNombre: lp.nombre, es_principal: lp.es_principal, lineaId: pl.id, precio: pl.precio });
  });
  const allTarifas = tarifasDisp ?? [];
  const [addingTarifaId, setAddingTarifaId] = useState<string | null>(null);
  const { data: addingListas } = useListasPrecioByTarifa(addingTarifaId ?? undefined);
  const [addPrices, setAddPrices] = useState<Record<string, number>>({});
  const handleSavePrices = async () => {
    if (!productoId || !addingTarifaId) return;
    try {
      for (const [listaId, precio] of Object.entries(addPrices)) { if (precio > 0) await savePrecio.mutateAsync({ lista_precio_id: listaId, producto_id: productoId, precio }); }
      setAddingTarifaId(null); setAddPrices({}); refetch(); toast.success('Precios guardados');
    } catch (err: any) { toast.error(err.message); }
  };
  const handleUpdatePrecio = async (lineaId: string, listaId: string, precio: number) => {
    if (!productoId) return;
    try { await savePrecio.mutateAsync({ id: lineaId, lista_precio_id: listaId, producto_id: productoId, precio }); refetch(); } catch (err: any) { toast.error(err.message); }
  };
  const handleDeletePrecio = async (lineaId: string) => { try { await deletePrecio.mutateAsync(lineaId); refetch(); } catch (err: any) { toast.error(err.message); } };
  if (isNew) return <div className="text-[12px] text-muted-foreground py-4 bg-accent/30 border border-accent/50 rounded px-3">💡 Guarda el producto primero para configurar listas de precios.</div>;
  return (
    <div className="space-y-3">
      {Array.from(byTarifa.entries()).map(([tarifaId, { tarifaNombre, listas: ls }]) => (
        <div key={tarifaId} className="border border-border rounded">
          <div className="px-3 py-1.5 bg-muted/30 border-b border-border text-[12px] font-semibold text-foreground">{tarifaNombre}</div>
          <table className="w-full text-sm"><tbody>
            {ls.map(l => (
              <tr key={l.listaId} className="border-b border-table-border last:border-0 hover:bg-table-hover">
                <td className="py-1.5 px-3"><span className="font-medium text-[13px]">{l.listaNombre}</span>{l.es_principal && <Crown className="h-3 w-3 text-primary inline ml-1.5" />}</td>
                <td className="py-1.5 px-3 text-right w-40"><InlinePriceEdit value={l.precio} onSave={(val) => l.lineaId && handleUpdatePrecio(l.lineaId, l.listaId, val)} /></td>
                <td className="py-1.5 px-3 w-10 text-center">{l.lineaId && <button onClick={() => handleDeletePrecio(l.lineaId!)} className="text-destructive hover:text-destructive/80"><Trash2 className="h-3.5 w-3.5" /></button>}</td>
              </tr>
            ))}
          </tbody></table>
        </div>
      ))}
      {byTarifa.size === 0 && !addingTarifaId && <div className="py-4 text-center text-[12px] text-muted-foreground border border-border rounded">Sin precios por lista configurados.</div>}
      {addingTarifaId && addingListas ? (
        <div className="border border-primary/30 rounded bg-primary/5 p-3 space-y-2">
          <div className="text-[13px] font-semibold">Agregar precios — {allTarifas.find((t: any) => t.id === addingTarifaId)?.nombre}</div>
          {addingListas.map(l => (
            <div key={l.id} className="flex items-center gap-3">
              <span className="text-[12px] flex-1">{l.nombre} {l.es_principal && <Crown className="h-3 w-3 text-primary inline ml-1" />}</span>
              <input type="number" className="inline-edit-input w-28 text-right" placeholder="$ 0.00" value={addPrices[l.id] || ''} onChange={e => setAddPrices(p => ({ ...p, [l.id]: +e.target.value }))} step="0.01" />
            </div>
          ))}
          {addingListas.length === 0 && <div className="text-[12px] text-muted-foreground">Esta tarifa no tiene listas de precios.</div>}
          <div className="flex gap-2 mt-2">
            <button onClick={handleSavePrices} disabled={savePrecio.isPending} className="btn-odoo-primary text-[12px] py-1"><Save className="h-3 w-3" /> Guardar</button>
            <button onClick={() => { setAddingTarifaId(null); setAddPrices({}); }} className="btn-odoo-secondary text-[12px] py-1">Cancelar</button>
          </div>
        </div>
      ) : (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[12px] text-muted-foreground">Agregar precios de:</span>
          {allTarifas.map((t: any) => <button key={t.id} onClick={() => setAddingTarifaId(t.id)} className="odoo-link text-[12px]"><Plus className="h-3 w-3 inline mr-0.5" />{t.nombre}</button>)}
        </div>
      )}
    </div>
  );
...
function InlinePriceEdit({ value, onSave }: { value: number; onSave: (v: number) => void }) {
  ...
  if (editing) return <input autoFocus type="number" className="inline-edit-input text-right w-full" value={draft} onChange={e => setDraft(e.target.value)} onBlur={() => { const v = parseFloat(draft); if (!isNaN(v) && v !== value) onSave(v); setEditing(false); }} onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur(); if (e.key === 'Escape') setEditing(false); }} step="0.01" />;
  return <span className="inline-edit-cell inline-edit-idle font-mono text-odoo-teal font-semibold cursor-pointer" onClick={() => { setDraft(value.toString()); setEditing(true); }}>$ {value.toFixed(2)}</span>;
}
