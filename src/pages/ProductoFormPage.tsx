...
/* ── Listas de Precios Tab (for product form) ── */
function ListasPrecioProductoTab({ productoId, isNew, tarifasDisp }: {
  productoId?: string; isNew: boolean; tarifasDisp: any;
}) {
  ...
  (precioLineas ?? []).forEach((pl: any) => {
    const lp = pl.lista_precios;
    if (!lp?.tarifas) return;
    const tarifaId = lp.tarifas.id;
    if (!byTarifa.has(tarifaId)) byTarifa.set(tarifaId, { tarifaNombre: lp.tarifas.nombre, listas: [] });
    byTarifa.get(tarifaId)!.listas.push({ listaId: lp.id, listaNombre: lp.nombre, es_principal: lp.es_principal, lineaId: pl.id, precio: pl.precio });
  });
  ...
  const handleSavePrices = async () => {
    ...
    try {
      for (const [listaId, precio] of Object.entries(addPrices)) {
        if (precio > 0) await savePrecio.mutateAsync({ lista_precio_id: listaId, producto_id: productoId, precio });
      }
      setAddingTarifaId(null); setAddPrices({}); refetch();
  ...
  const handleDeletePrecio = async (lineaId: string) => {
    try { await deletePrecio.mutateAsync(lineaId); refetch(); } catch (err: any) { toast.error(err.message); }
  };
  ...
  return (
    <div className="space-y-3">
            ...
            <tbody>
              {ls.map(l => (
                <tr key={l.listaId} className="border-b border-table-border last:border-0 hover:bg-table-hover">
                  <td className="py-1.5 px-3">
                    <span className="font-medium text-[13px]">{l.listaNombre}</span>
                    {l.es_principal && <Crown className="h-3 w-3 text-primary inline ml-1.5" />}
                  </td>
                  <td className="py-1.5 px-3 text-right w-40">
                    <InlinePriceEdit value={l.precio} onSave={(val) => l.lineaId && handleUpdatePrecio(l.lineaId, l.listaId, val)} />
                  </td>
                  <td className="py-1.5 px-3 w-10 text-center">
                    {l.lineaId && (
                      <button onClick={() => handleDeletePrecio(l.lineaId!)} className="text-destructive hover:text-destructive/80">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    )}
                  </td>
      ...
      {byTarifa.size === 0 && !addingTarifaId && (
        <div className="py-4 text-center text-[12px] text-muted-foreground border border-border rounded">Sin precios por lista configurados.</div>
      )}
      {addingTarifaId && addingListas ? (
        <div className="border border-primary/30 rounded bg-primary/5 p-3 space-y-2">
          ...
          {addingListas.map(l => (
            <div key={l.id} className="flex items-center gap-3">
              ...
              <input type="number" className="inline-edit-input w-28 text-right" placeholder="$ 0.00"
                value={addPrices[l.id] || ''} onChange={e => setAddPrices(p => ({ ...p, [l.id]: +e.target.value }))} step="0.01" />
            </div>
          ))}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {allTarifas.map((t: any) => (
            <button key={t.id} onClick={() => setAddingTarifaId(t.id)} className="odoo-link text-[12px]">
              <Plus className="h-3 w-3 inline mr-0.5" />{t.nombre}
            </button>
          ))}
        </div>
      )}
...
function InlinePriceEdit({ value, onSave }: { value: number; onSave: (v: number) => void }) {
  ...
  if (editing) {
    return (
      <input autoFocus type="number" className="inline-edit-input text-right w-full" value={draft}
        onChange={e => setDraft(e.target.value)}
        onBlur={() => { const v = parseFloat(draft); if (!isNaN(v) && v !== value) onSave(v); setEditing(false); }}
        onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur(); if (e.key === 'Escape') setEditing(false); }}
        step="0.01" />
    );
  }
  return (
    <span className="inline-edit-cell inline-edit-idle font-mono text-odoo-teal font-semibold cursor-pointer"
