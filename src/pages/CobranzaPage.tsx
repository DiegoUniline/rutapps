...
export default function CobranzaPage() {
  ...
  const selectAllVisible = () => {
    const visibleIds = pagination.paginatedItems.map(c => c.id);
  ...
  const handleWaMany = () => {
    // Build a combined message for all selected
    ...
    // For multiple, we can't send one WA message with multiple receipts easily;
    // just open for the first one and let user send individually
    openWaCobro(selectedCobros[0]);
