...
export default function SolicitudCompraProveedorPublicaPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        setLines((next.lineas || []).map(line => ({
          ...
          cantidad_surtida: line.disponible === false ? 0 : (line.cantidad_surtida ?? line.cantidad_solicitada),
        })));
  ...
  const setUnavailable = (line: SupplierLine, unavailable: boolean) => {
    if (readOnly) return;
    updateLine(line.id, unavailable
      ? { disponible: false, cantidad_surtida: 0, costo_unitario: null }
      : {
          disponible: true,
          cantidad_surtida: Number(line.cantidad_surtida || 0) > 0 ? line.cantidad_surtida : line.cantidad_solicitada,
        });
  };
    ...
    <PublicShell reserveBottom={!readOnly}>
      <main className={`mx-auto w-full max-w-[1160px] px-3 pt-5 sm:px-5 sm:pt-8 ${readOnly ? 'pb-8' : 'pb-32'}`}>
          ...
          <div className="p-5 sm:p-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[#667085]"><strong className="text-[#172033]">{data.empresa.nombre}</strong> solicita confirmar disponibilidad, cantidad y costo.</p>
              </div>
          ...
          <section className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-4 sm:px-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-5 text-emerald-800">Tu respuesta ya fue registrada. Puedes consultar disponibilidad, cantidades, unidades, costos y observaciones, pero no modificarlas.</p>
              ...
              <article key={line.id} className={`rounded-2xl border bg-white p-4 shadow-sm transition ${available ? 'border-[#e5eaf1]' : 'border-red-200 bg-red-50/30'}`}>
                <div className="grid gap-4 lg:grid-cols-[minmax(210px,1.45fr)_100px_100px_125px_135px_130px_minmax(150px,1fr)]">
                  ...
                  <Field label="No lo tengo">
                    <label className={`flex min-h-[40px] items-center gap-2 rounded-[10px] border px-3 text-xs font-black ${available ? 'border-[#d0d5dd] bg-white text-[#475467]' : 'border-red-200 bg-red-50 text-red-700'} ${readOnly ? 'cursor-default' : 'cursor-pointer'}`}>
                      <input disabled={readOnly} type="checkbox" className="h-4 w-4 accent-red-600" checked={!available} onChange={e => setUnavailable(line, e.target.checked)} />
                      {available ? 'No' : 'Sí'}
                    </label>
                  </Field>
        ...
        <section className="mt-4 rounded-2xl border border-[#e5eaf1] bg-white p-4 sm:p-5">
          ...
          <textarea disabled={readOnly} className="public-input mt-2 min-h-[90px] resize-y" value={generalNote} onChange={e => !readOnly && setGeneralNote(e.target.value)} placeholder="Ej. disponibilidad parcial, cambio de presentación o alguna condición del proveedor…" />
        </section>
          ...
          <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/70 bg-white shadow-[0_28px_80px_rgba(16,24,40,.28)]">
            ...
            <div className="p-5 sm:p-6">
              <div className="flex items-start gap-4">
                <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#eef5ff] text-[#0061e8]"><LockKeyhole className="h-6 w-6" /></div>
                ...
                <button type="button" onClick={() => setCloseConfirmOpen(false)} className="grid h-9 w-9 shrink-0 place-items-center rounded-xl text-[#98a2b3] transition hover:bg-[#f2f4f7] hover:text-[#344054]" aria-label="Cerrar confirmación"><X className="h-4 w-4" /></button>
              </div>
              ...
              <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-amber-800">Podrás seguir consultando este mismo enlace, pero no modificar disponibilidad, cantidades, costos ni observaciones. Si necesitas corregir algo, tu cliente deberá abrir la solicitud para modificar desde RutApp.</p>
                  </div>
                </div>
              </div>
