  ...
  const handleCobrar = async () => {
    ...
    try {
      ...
      setLastVentaData({
        ...
        pagos: (paySplitsComputed.length > 0 ? paySplitsComputed : [{ metodo: 'efectivo', monto: totals.total }]).map(s => ({ metodo: s.metodo, monto: (s as any).monto ?? totals.total, fecha: fmtDate(todayLocal()) })),
      });
