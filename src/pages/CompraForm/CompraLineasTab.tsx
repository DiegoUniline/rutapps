            ...
            {lineas.map((line, idx) => {
              ...
              return (
                <tr key={idx} className="border-b border-table-border" data-row={idx}>
                  ...
                  <td className="py-1.5 px-2"><input type="number" className="input-odoo w-24 text-right text-sm" value={line.cantidad ?? 1} onChange={e => updateLinea(idx, 'cantidad', Number(e.target.value))} disabled={!isEditable} min={0} /></td>
                  <td className="py-1.5 px-1"><input type="number" className="w-20 text-center text-sm bg-transparent border border-border rounded px-2 py-1 tabular-nums focus:outline-none focus:ring-1 focus:ring-primary" value={line._factor_conversion ?? 1} onChange={e => updateLinea(idx, '_factor_conversion', Math.max(1, Number(e.target.value) || 1))} disabled={!isEditable} min={1} /></td>
                  ...
                  <td className="py-1.5 px-3"><input type="number" className="input-odoo w-28 text-right text-sm" value={line.precio_unitario ?? 0} onChange={e => updateLinea(idx, 'precio_unitario', Number(e.target.value))} disabled={!isEditable} step="0.01" /></td>
