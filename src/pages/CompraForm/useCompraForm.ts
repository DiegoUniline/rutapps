  ...
  const handleCancel = async () => {
    ...
    try {
      if (['recibida', 'pagada'].includes(form.status)) {
        ...
        for (let i = 0; i < validLines.length; i++) { const l = validLines[i]; const factor = Number(l._factor_conversion) || 1; const piezas = (Number(l.cantidad) || 0) * factor; const currentQty = Number(prodResults[i].data?.cantidad ?? 0); updates.push(supabase.from('productos').update({ cantidad: Math.max(0, currentQty - piezas) } as any).eq('id', l.producto_id!).then(() => {})); updates.push(supabase.from('movimientos_inventario').insert({ empresa_id: empresa!.id, tipo: 'salida', producto_id: l.producto_id!, cantidad: piezas, almacen_origen_id: form.almacen_id, referencia_tipo: 'compra', referencia_id: form.id, user_id: user?.id, fecha: today, notas: `Cancelación compra ${form.folio ?? form.id.slice(0, 8)}` } as any).then(() => {})); }
