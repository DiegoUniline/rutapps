          ...
          <tbody>
            {pagos.map(p => (
              <tr key={p.id} className="border-b border-table-border">
                ...
                <td className="py-1.5 px-3">{form.status !== 'pagada' && <button onClick={async () => { if (!await confirmDialog('¿Eliminar este pago?')) return; await supabase.from('pago_compras').delete().eq('id', p.id); const nuevoSaldo = Math.max(0, totals.total - (totalPagado - p.monto)); await supabase.from('compras').update({ saldo_pendiente: nuevoSaldo } as any).eq('id', form.id); qc.invalidateQueries({ queryKey: ['pagos-compra', form.id] }); toast.success('Pago eliminado'); }} className="text-destructive hover:text-destructive/80"><Trash2 className="h-3.5 w-3.5" /></button>}</td>
              </tr>
            ))}
            {addingPago && (
