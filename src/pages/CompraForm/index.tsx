...
export default function CompraFormPage() {
  ...
  return (
    <><div className="p-3 sm:p-4 space-y-4 min-h-full max-w-full overflow-x-hidden">
      <CompraHeader form={h.form} isNew={h.isNew} isEditable={h.isEditable} editingExisting={h.editingExisting} saving={h.saving} cancelling={h.cancelling} dirty={h.dirty} totalPagado={h.totalPagado} totals={h.totals} saldoActual={h.saldoActual} hayPendienteRecibir={hayPendienteRecibir} confirmDialog={h.confirmDialog} setConfirmDialog={h.setConfirmDialog} handleSave={h.handleSave} beginEditing={h.beginEditing} cancelEditing={h.cancelEditing} handleDelete={h.handleDelete} handleStatusChange={h.handleStatusChange} handleCancel={h.handleCancel} recibirTodoPendiente={h.recibirTodoPendiente} requestPin={h.requestPin} onBack={() => h.navigate('/almacen/compras')} onRegistrarPago={() => { h.setNewPago(() => ({ fecha: todayInTimezone(empresa?.zona_horaria), metodo_pago: 'transferencia', referencia: '', notas: '', monto: h.saldoActual })); h.setAddingPago(true); setActiveTab('pagos'); }} />
