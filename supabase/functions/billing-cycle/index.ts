...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ═══ PART 4: Reconcile legacy retry rows safely (daily) ═══
    // Stripe is the ONLY engine allowed to retry/collect Stripe invoices.
    // Rutapp never calls invoices.pay() for a pending retry anymore.
    if (stripe) {
      const { data: retries } = await supabase
        .from("cobro_reintentos")
        .select("id, factura_id, empresa_id, stripe_invoice_id, intento_num, facturas:factura_id(id, stripe_invoice_id, total, estado)")
        .eq("estado", "pendiente")
        .lte("proxima_fecha", today);

      log("Legacy retries to reconcile", { count: retries?.length || 0 });

      for (const retry of retries || []) {
        const factura = (retry.facturas as any) || null;
        const stripeInvId = retry.stripe_invoice_id || factura?.stripe_invoice_id;

        if (!stripeInvId) {
          await supabase.from("cobro_reintentos")
            .update({
              estado: "fallido",
              ultimo_error: "Sin stripe_invoice_id; no se intentó ningún cobro",
        ...
        try {
          let inv = await stripe.invoices.retrieve(stripeInvId);

          // Orphan local invoice: safest interpretation of an old admin delete is
          // to guarantee that Stripe cannot collect it later.
          if (!factura) {
            if (inv.status === "draft") {
              await stripe.invoices.update(stripeInvId, { auto_advance: false });
              await stripe.invoices.del(stripeInvId);
            } else if (inv.status === "open") {
              await stripe.invoices.update(stripeInvId, { auto_advance: false });
              inv = await stripe.invoices.voidInvoice(stripeInvId);
            }

            await supabase.from("cobro_reintentos")
              .update({
                estado: "procesado",
                ultimo_error: "Detenido por seguridad: factura local inexistente; Stripe neutralizado",
                procesado_at: now.toISOString(),
              })
              .eq("id", retry.id);
            log("Orphan retry neutralized", { retryId: retry.id, invoiceId: stripeInvId, stripeStatus: inv.status });
            continue;
          }

          const localStatus = String(factura.estado || "").toLowerCase();

          // Existing bad state repair: local invoice was manually marked paid,
          // but Stripe may still be open. Close it paid_out_of_band without
          // charging the saved card.
          if (localStatus === "pagada") {
            if (inv.status === "draft") {
              await stripe.invoices.update(stripeInvId, { auto_advance: false });
              inv = await stripe.invoices.finalizeInvoice(stripeInvId);
            }
            if (inv.status === "open") {
              await stripe.invoices.update(stripeInvId, { auto_advance: false });
              inv = await stripe.invoices.pay(stripeInvId, { paid_out_of_band: true });
            }

            await supabase.from("cobro_reintentos")
              .update({
                estado: inv.status === "paid" ? "exitoso" : "procesado",
                ultimo_error: inv.status === "paid"
                  ? "Sincronizado: factura local pagada; Stripe cerrado paid_out_of_band"
                  : `Factura local pagada; Stripe ya estaba en estado terminal ${inv.status}`,
                procesado_at: now.toISOString(),
              })
              .eq("id", retry.id);
            log("Paid local invoice reconciled without charge", { retryId: retry.id, invoiceId: stripeInvId, stripeStatus: inv.status });
            continue;
          }

          // Existing bad state repair: cancelled local invoice must never remain
          // collectible in Stripe.
          if (["cancelada", "cancelado", "cancelled", "canceled"].includes(localStatus)) {
            if (inv.status === "draft") {
              await stripe.invoices.update(stripeInvId, { auto_advance: false });
              await stripe.invoices.del(stripeInvId);
            } else if (inv.status === "open") {
            ...
            await supabase.from("cobro_reintentos")
              .update({
                estado: "procesado",
                ultimo_error: "Detenido: factura cancelada; Stripe neutralizado",
                procesado_at: now.toISOString(),
              })
              .eq("id", retry.id);
            log("Cancelled invoice retry neutralized", { retryId: retry.id, invoiceId: stripeInvId, stripeStatus: inv.status });
            continue;
          }

          // Stripe may already have collected it through its own retry engine.
          if (inv.status === "paid") {
            ...
            await supabase.from("cobro_reintentos")
              .update({
                estado: "exitoso",
                ultimo_error: "Stripe ya confirmó el pago; Rutapp no ejecutó cobro adicional",
                procesado_at: now.toISOString(),
              })
              .eq("id", retry.id);
            continue;
          ...
          // Terminal/non-collectable Stripe states: stop the local retry chain.
          if (inv.status === "void" || inv.status === "uncollectible") {
            await supabase.from("cobro_reintentos")
              .update({
                estado: "procesado",
                ultimo_error: `Stripe en estado ${inv.status}; Rutapp no ejecutó cobro`,
                procesado_at: now.toISOString(),
              })
              .eq("id", retry.id);
            continue;
          ...
          // Open/draft/other: Stripe owns its automatic retry schedule. Rutapp
          // explicitly does NOT call stripe.invoices.pay() here.
          await supabase.from("cobro_reintentos")
            .update({
              estado: "procesado",
              ultimo_error: `Delegado a Stripe (estado ${inv.status || "desconocido"}); Rutapp no forzó cobro`,
              procesado_at: now.toISOString(),
            })
            .eq("id", retry.id);
          log("Retry delegated to Stripe", { retryId: retry.id, invoiceId: stripeInvId, stripeStatus: inv.status });
        } catch (e) {
          const msg = (e as Error).message;
          log("Retry reconciliation failed safely", { retryId: retry.id, error: msg });
          await supabase.from("cobro_reintentos")
            .update({
              estado: "fallido",
              ultimo_error: `Reconciliación falló; no se ejecutó cobro desde Rutapp: ${msg}`,
              procesado_at: now.toISOString(),
            })
            .eq("id", retry.id);
        }
