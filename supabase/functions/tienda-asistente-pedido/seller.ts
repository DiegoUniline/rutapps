...
async function writeSellerResponse(
  ...
  const fallback = () => {
    ...
    if (/hola|buenas|buen dia|buen día/i.test(input.text)) return `Hola${input.cliente?.contacto ? `, ${input.cliente.contacto}` : input.cliente?.nombre ? `, ${input.cliente.nombre}` : ""}. Claro, te ayudo a preparar tu pedido. ¿Qué necesitas hoy?`;
...
export async function handleSellerRequest(args: {
  ...
  if (body?.action === "welcome") {
    ...
    return {
      message: `${greeting} Soy tu asesor de ${storeName}.${last}`,
