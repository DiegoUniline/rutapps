...
BEGIN
  ...
  out := out || jsonb_build_object('paso','1 venta 10 pzas','deltaA',
  ...
  out := out || jsonb_build_object('paso','2 editar 10->4','deltaA',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pa)-ini_a);

  UPDATE venta_lineas SET producto_id=v_pb WHERE id=v_lid;
  out := out || jsonb_build_object('paso','3 sustituir A->B','deltaA',
  ...
  out := out || jsonb_build_object('paso','4 a borrador','deltaA',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pa)-ini_a,'deltaB',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pb)-ini_b);

  INSERT INTO venta_lineas (venta_id,empresa_id,producto_id,cantidad,precio_unitario,subtotal,total)
  VALUES (v_vid,v_emp,v_pa,3,100,300,300);
  out := out || jsonb_build_object('paso','5 linea nueva en borrador','deltaA',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pa)-ini_a);

  UPDATE ventas SET status='entregado' WHERE id=v_vid;
  out := out || jsonb_build_object('paso','6 re-confirmar','deltaA',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pa)-ini_a,'deltaB',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pb)-ini_b);

  UPDATE ventas SET status='entregado' WHERE id=v_vid;
  UPDATE ventas SET status='entregado' WHERE id=v_vid;
  out := out || jsonb_build_object('paso','7 idempotencia x2','deltaA',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pa)-ini_a,'deltaB',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pb)-ini_b);

  DELETE FROM venta_lineas WHERE id=v_lid;
  out := out || jsonb_build_object('paso','8 borrar linea B','deltaB',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pb)-ini_b);

  UPDATE ventas SET status='cancelado' WHERE id=v_vid;
  out := out || jsonb_build_object('paso','9 cancelar','deltaA',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pa)-ini_a,'deltaB',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pb)-ini_b);

  out := out || jsonb_build_object('paso','movimientos',
    (SELECT jsonb_agg(jsonb_build_object('t',tipo,'ref',referencia_tipo,'cant',cantidad) ORDER BY created_at)
  ...
  out := out || jsonb_build_object('paso','10 limpieza','deltaA',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pa)-ini_a,'deltaB',
    (SELECT cantidad FROM stock_almacen WHERE almacen_id=v_alm AND producto_id=v_pb)-ini_b);
  RETURN out;
END;
...
GRANT EXECUTE ON FUNCTION public.__test_sync_venta_edicion() TO service_role;