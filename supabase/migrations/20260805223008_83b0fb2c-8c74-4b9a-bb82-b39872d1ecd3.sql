...
BEGIN
  ...
  -- Deseado por producto/lote (lote explícito de la línea o de venta_linea_lotes)
  ...
  DELETE FROM _sync_des; DELETE FROM _sync_apl;
  ...
  LOOP
    ...
    -- 1) Lotes explícitamente deseados: ajustar cada uno a su cantidad exacta
    FOR l IN
      SELECT d.lote_id,
    ...
    -- 2) Resto (sin lote explícito): FEFO o movimiento sin lote
    ...
    ELSIF v_res < -0.000001 THEN
      v_res := -v_res;
      -- devolver primero de los lotes aplicados que ya no se desean
      FOR l IN
