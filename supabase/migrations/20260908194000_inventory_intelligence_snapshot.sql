-- Inventario: snapshot agregado para evitar descargar miles de ventas y renglones
-- al navegador. SECURITY INVOKER conserva todas las políticas RLS existentes.

CREATE INDEX IF NOT EXISTS idx_movimientos_inventario_empresa_fecha
  ON public.movimientos_inventario (empresa_id, fecha DESC);

CREATE INDEX IF NOT EXISTS idx_stock_lotes_empresa_positivo
  ON public.stock_lotes (empresa_id, producto_id, almacen_id)
  WHERE cantidad > 0;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'movement_type_counts', COALESCE((SELECT jsonb_object_agg(mt.tipo, mt.cantidad) FROM movement_totals mt), '{}'::jsonb),
...
COMMENT ON FUNCTION public.fn_inventory_intelligence_snapshot(uuid, integer)
  IS 'Snapshot agregado y de solo lectura para el panel de inteligencia de inventario.';
...
ANALYZE public.movimientos_inventario;
ANALYZE public.stock_lotes;
