...
BEGIN
  ...
  IF v_rec.almacen_id IS NOT NULL THEN
    ...
    END IF;
    -- NOTA: el stock por lote lo aplica trg_sync_stock_lote al insertar el movimiento.
  END IF;
...
-- Corrige el stock por lote duplicado por loteos de compras previos
WITH dup AS (
  SELECT cll.almacen_id, cll.lote_id, SUM(cll.piezas) AS piezas
  FROM compra_linea_lotes cll
  WHERE cll.almacen_id IS NOT NULL
  GROUP BY cll.almacen_id, cll.lote_id
)
UPDATE stock_lotes sl
   SET cantidad = GREATEST(0, sl.cantidad - dup.piezas), updated_at = now()
  FROM dup
 WHERE sl.almacen_id = dup.almacen_id AND sl.lote_id = dup.lote_id;